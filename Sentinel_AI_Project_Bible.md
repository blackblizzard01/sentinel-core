...
MutationAgent:  OpenAI — gpt-4o-mini
ReportAgent:    Anthropic — claude-sonnet-4-20250514
AutopatchAgent: Anthropic — claude-sonnet-4-20250514
...
self.openai    # AsyncOpenAI client

# Methods available to every agent
async def call_groq(prompt, system, temperature) → str
async def call_claude(prompt, system, max_tokens) → str
async def call_openai(prompt, system, temperature) → str
