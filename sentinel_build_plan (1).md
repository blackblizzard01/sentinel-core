...
Each team member creates accounts and obtains API keys for: Groq (free tier), Google Gemini (Google AI Studio), DeepSeek. Keys are personal and must NOT be committed to the repository. Post confirmation of all three keys obtained in Discord `#setup-log`.
...
### W1-TEAM-04 — .env Setup for Shared Secrets
**Type:** Team Task
**Owner:** CA
...
Copy `.env.example` to `.env` in each local environment. Populate all environment variables with real values (Groq, Gemini, DeepSeek keys, Supabase URL, Upstash Redis URL, JWT secret). Ensure `.env` is listed in `.gitignore` — never commit it. Document the local setup pattern (`cp .env.example .env`, then fill in keys) in README. Verify env vars are loaded by running `python -c "import os; from dotenv import load_dotenv; load_dotenv(); print(os.getenv('GROQ_API_KEY_1'))"` — must return a non-null value.
...
**Type:** Team Task
**Owner:** CA
**Depends on:** W1-TEAM-04 (.env setup), W1-TEAM-05 (DB models for import)
...
- [ ] **W1-TEAM-04** — `.env` file populated with all required keys; `python -c "import os; from dotenv import load_dotenv; load_dotenv(); print(os.getenv('GROQ_API_KEY_1'))"` returns a non-null value; `.env` confirmed absent from git tracking.
...
Flesh out `agents/base_agent.py` from skeleton to full implementation. `call_groq`, `call_gemini`, `call_deepseek` must make real async API calls, handle rate limit exceptions with exponential backoff (max 3 retries), and return the response string. `log_action` and `log_error` must write structured entries to Python `logging` module with agent name, scan ID, and timestamp. Add `tests/test_base_agent.py` verifying that a `call_groq` invocation with a simple prompt returns a non-empty string.

---

### W2-INFRA-01 — ApiKeyManager (COMPLETED)
**File:** agents/api_key_manager.py
- Round-robin rotation across numbered .env slots
- Thread-safe for concurrent agents
- acquire_key(provider) / rotate(provider) interface
- Integrated into BaseAgent.__init__ and retry handlers
Status: ✅ Done — all agents inherit automatically via BaseAgent
...
Implement `agents/mutation_agent.py` inheriting `BaseAgent`. Build four mutation strategy functions, each taking a parent payload string and returning a mutated string via LLM call to DeepSeek:
...
Implement `generate_executive_summary(aggregated_findings: dict, client_name: str) -> str` using `call_gemini` with Google Gemini. System prompt instructs Gemini to write a 3-paragraph executive summary for a non-technical audience covering: overall risk posture, top three most critical findings, and recommended immediate actions. User prompt injects the structured findings dict. Output must contain: overall risk level (Critical/High/Medium/Low), specific component names from the scan, and at least one concrete remediation recommendation. Add a 60-second timeout — if Gemini takes longer than 60 seconds, raise a `ReportGenerationTimeout` exception.
